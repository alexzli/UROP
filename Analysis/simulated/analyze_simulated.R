...
results <- readRDS(file.path(datadir, '/data/sim_puck_results.rds'))
...
#                 ((berg_purk_pred$first_type == '4' & berg_purk_pred$second_type == '8') | (berg_purk_pred$first_type == '8' & berg_purk_pred$second_type == '4')),]

#berg_first_IDs <- rownames(berg_purk_pred_doublets[berg_purk_pred_doublets$first_type == '8',])
#berg_second_IDs <- rownames(berg_purk_pred_doublets[berg_purk_pred_doublets$second_type == '8',])

berg_first_IDs <- rownames(berg_purk_pred[berg_purk_pred$first_type == '8',])
berg_second_IDs <- rownames(berg_purk_pred[berg_purk_pred$second_type == '8',])
...
# MEAN SQUARED ERROR OF DOUBLET PROPORTIONS
...
dim(berg_purk_pred[berg_purk_pred$spot_class == 'singlet' & 
               (berg_purk_pred$first_type == '4' | berg_purk_pred$first_type == '8'),])
...
doublets_pred <- pred_results[pred_results$spot_class == 'doublet_certain',]
doublets_true <- results[rownames(doublets_pred),]
doublets_true$prop <- strtoi(pmin(doublets_true$UMI1, doublets_true$UMI2)) / 1000
results$prop <- strtoi(pmin(results$UMI1, results$UMI2)) / 1000
