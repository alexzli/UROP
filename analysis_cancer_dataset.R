...
original_info <- myRCTD@cell_type_info$renorm
...
cancer_info <- get_cell_type_info(puck@counts, cell_types, colSums(puck@counts))[[1]]

# spatially plot cancer cells
...
plot1 <- DimPlot(slide.seq.clusters, reduction = "pca")
plot1

# single class cancer classification
cancer_info <- cancer_info[1:1]
original_info[[1]] <- original_info[[1]][rownames(cancer_info),]
original_info[[1]] <- sweep(original_info[[1]], 2, colSums(original_info[[1]]), '/')
new_info <- list(cbind(original_info[[1]], cancer_info), append(original_info[[2]], 'Cancer_cells'), original_info[[3]]+1)
